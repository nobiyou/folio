...
class Folio_AI_API_Manager {
    ...
    public function call_api($prompt, $options = array()) {
        if (empty($this->apis)) {
            error_log('Folio AI API Manager: ' . __('No available API configuration', 'folio'));
            return false;
        }
        ...
        error_log('Folio AI API Manager: ' . __('All API calls failed', 'folio'));
        return false;
    }
    ...
    private function try_call_api($api, $prompt, $options = array()) {
        ...
        // 记录错误
        $error_message = __('Unable to parse response', 'folio');
        if (isset($data['error'])) {
            $error_message = sprintf(
                /* translators: %s: API error details. */
                __('API error: %s', 'folio'),
    ...
    public function test_connection($api_index = null) {
        ...
        if ($api_index !== null) {
            ...
            if (isset($this->apis[$api_index])) {
                ...
                $test_prompt = __('Please reply: connection successful', 'folio');
                $result = $this->try_call_api($api, $test_prompt);
                $results[$api_index] = array(
                    'name' => $api['name'] ?: sprintf(__('API Config #%d', 'folio'), $api_index + 1),
                    'endpoint' => $api['endpoint'],
                    'model' => $api['model'],
                    'success' => is_array($result) && isset($result['success']) ? $result['success'] : false,
                    'message' => is_array($result) && isset($result['success']) && $result['success'] ? __('Connection successful', 'folio') : (is_array($result) && isset($result['error']) ? $result['error'] : __('Connection failed', 'folio')),
            ...
            foreach ($this->apis as $index => $api) {
                ...
                $result = $this->try_call_api($api, $test_prompt);
                $results[$index] = array(
                    'name' => $api['name'] ?: sprintf(__('API Config #%d', 'folio'), $index + 1),
                    'endpoint' => $api['endpoint'],
                    'model' => $api['model'],
                    'success' => is_array($result) && isset($result['success']) ? $result['success'] : false,
                    'message' => is_array($result) && isset($result['success']) && $result['success'] ? __('Connection successful', 'folio') : (is_array($result) && isset($result['error']) ? $result['error'] : __('Connection failed', 'folio')),
