...
class Folio_AI_Content_Generator {
    ...
    public function add_ai_meta_box() {
        ...
        foreach ($post_types as $post_type) {
            add_meta_box(
                'folio_ai_generator',
                '<span class="dashicons dashicons-admin-generic"></span> ' . __('AI Content Generator', 'folio'),
    ...
    public function render_ai_meta_box($post) {
        if (!$this->api_manager->has_apis()) {
            ...
            <div class="folio-ai-notice notice-warning">
                <p><?php esc_html_e('Please configure AI API in Theme Settings first', 'folio'); ?></p>
        ...
        <div class="folio-ai-generator">
            <div class="folio-ai-options">
                <label>
                    ...
                    <?php esc_html_e('Auto-generate on save', 'folio'); ?>
                </label>
            </div>
            ...
            <div class="folio-ai-status" style="display: none;">
                <div class="folio-ai-loading">
                    ...
                    <span class="folio-ai-status-text"><?php esc_html_e('AI is generating...', 'folio'); ?></span>
                </div>
            </div>
    ...
    public function enqueue_admin_scripts($hook) {
        ...
        wp_localize_script('folio-ai-generator', 'folioAI', array(
            ...
            'post_id' => get_the_ID(),
            'strings' => array(
                'generating' => __('AI is generating...', 'folio'),
                'success' => __('Generated successfully!', 'folio'),
                'error' => __('Generation failed. Please try again.', 'folio'),
                'no_content' => __('Please enter title and content first', 'folio'),
                'excerpt_generated' => __('Excerpt generated', 'folio'),
                'keywords_generated' => __('Keywords generated', 'folio'),
                'error_label' => __('Error: ', 'folio'),
            ),
        ));
    }
    ...
    public function ajax_generate_content() {
        ...
        if (empty($title) && empty($content)) {
            wp_send_json_error(array('message' => __('Please enter title and content first', 'folio')));
        }
        ...
        if (!empty($result)) {
            ...
            wp_send_json_error(array('message' => __('AI generation failed. Check API configuration or site error logs.', 'folio')));
        }
    }
...
Return only the JSON object, no other content.";
    ...
    public function ajax_test_connection() {
        ...
        if (!$this->api_manager->has_apis()) {
            wp_send_json_error(array('message' => __('Please configure API first', 'folio')));
        }
        ...
        foreach ($results as $index => $result) {
            if ($result['success']) {
                $success_count++;
                $messages[] = sprintf(__('%s: Connection successful', 'folio'), $result['name'] ?: 'API #' . ($index + 1));
            } else {
                $error_msg = isset($result['error']) ? $result['error'] : (isset($result['message']) ? $result['message'] : __('Unknown error', 'folio'));
                $messages[] = sprintf(__('%s: Connection failed - %s', 'folio'), $result['name'] ?: 'API #' . ($index + 1), $error_msg);
            }
        }
        
        if ($success_count > 0) {
            wp_send_json_success(array(
                'message' => sprintf(__('Test completed: %d/%d APIs connected successfully', 'folio'), $success_count, count($results)),
                'details' => $messages,
                'results' => $results,
            ));
        } else {
            wp_send_json_error(array(
                'message' => __('All API connections failed. Please check configuration.', 'folio'),
                'details' => $messages,
                'results' => $results,
            ));
        }
