...
if (!function_exists('folio_format_human_duration_for_dashboard')) {
    function folio_format_human_duration_for_dashboard($from_timestamp, $to_timestamp = null) {
        $to_timestamp = $to_timestamp ?: current_time('timestamp');
        $diff = max(0, (int) $to_timestamp - (int) $from_timestamp);

        $units = array(
            YEAR_IN_SECONDS => array(
                'singular' => __('%s year', 'folio'),
                'plural'   => __('%s years', 'folio'),
            ),
            MONTH_IN_SECONDS => array(
                'singular' => __('%s month', 'folio'),
                'plural'   => __('%s months', 'folio'),
            ),
            DAY_IN_SECONDS => array(
                'singular' => __('%s day', 'folio'),
                'plural'   => __('%s days', 'folio'),
            ),
            HOUR_IN_SECONDS => array(
                'singular' => __('%s hour', 'folio'),
                'plural'   => __('%s hours', 'folio'),
            ),
            MINUTE_IN_SECONDS => array(
                'singular' => __('%s minute', 'folio'),
                'plural'   => __('%s minutes', 'folio'),
            ),
        );
        ...
        foreach ($units as $seconds => $labels) {
            if ($diff >= $seconds) {
                ...
                $format = $value > 1 ? $labels['plural'] : $labels['singular'];
                return sprintf($format, number_format_i18n($value));
            }
        }

        return __('just now', 'folio');
    }
}
?>
...
<div class="dashboard">
    <h1 class="dashboard-title text-2xl font-bold mb-6">
        <?php printf(esc_html__('Welcome back, %s', 'folio'), wp_get_current_user()->display_name); ?>
        ...
        <div class="flex items-center justify-between flex-wrap gap-4">
            <div class="flex items-center gap-4">
                ...
                <div>
                    ...
                    <p class="membership-card-subtitle"><?php esc_html_e('Lifetime Member', 'folio'); ?></p>
                    <?php elseif ($membership['days_left'] !== null) : ?>
                    <p class="membership-card-subtitle">
                        <?php 
                        if ($membership['days_left'] > 0) {
                            printf(esc_html__('%d days remaining', 'folio'), $membership['days_left']);
                        } else {
            ...
            <div class="membership-benefits">
                <span class="membership-benefits-label text-sm"><?php esc_html_e('Membership Benefits', 'folio'); ?></span>
                <div class="flex gap-2 mt-1">
                    <span class="benefit-tag">✓ <?php esc_html_e('Exclusive Badge', 'folio'); ?></span>
                    <?php if ($membership['is_svip']) : ?>
                    <span class="benefit-tag">✓ <?php esc_html_e('Priority Support', 'folio'); ?></span>
                    <?php endif; ?>
                </div>
            </div>
        </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
        <div class="stat-card">
            ...
            <div>
                ...
                <div class="stat-label"><?php esc_html_e('Favorites', 'folio'); ?></div>
            </div>
        </div>
        ...
        <div class="stat-card">
            ...
            <div>
                <div class="stat-number"><?php echo esc_html(folio_format_human_duration_for_dashboard(strtotime(wp_get_current_user()->user_registered))); ?></div>
                <div class="stat-label"><?php esc_html_e('Joined Date', 'folio'); ?></div>
            </div>
        </div>
        ...
        <div class="stat-card stat-card-membership <?php echo $membership['is_vip'] ? 'stat-card-' . esc_attr($membership['level']) : ''; ?>">
            ...
            <div>
                ...
                <div class="stat-label"><?php esc_html_e('Membership Level', 'folio'); ?></div>
            </div>
        </div>
    </div>
    ...
    <div class="recent-favorites">
        <div class="flex items-center justify-between mb-4">
            <h2 class="recent-favorites-title text-xl font-bold"><?php esc_html_e('Recent Favorites', 'folio'); ?></h2>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            ...
            <div class="favorite-item">
                <a href="<?php echo get_permalink($post_id); ?>" class="block">
                    <?php
                    echo folio_render_thumbnail_block($post_id, 'medium', array(
                        'wrapper_class' => 'favorite-thumb',
                        'image_class' => 'w-full h-32 object-cover rounded-lg',
                        'placeholder_class' => 'w-full h-32 rounded-lg bg-gradient-to-br from-gray-200 to-gray-300',
                    ));
                    ?>
    ...
    <div class="empty-state text-center py-12">
        ...
        <h3 class="empty-state-title text-lg font-medium mb-2"><?php esc_html_e('No favorites yet', 'folio'); ?></h3>
        <p class="empty-state-text mb-4"><?php esc_html_e('Go explore some amazing works!', 'folio'); ?></p>
        <a href="<?php echo esc_url(home_url('/')); ?>" class="btn-primary">
            <?php esc_html_e('Browse Works', 'folio'); ?>
        </a>
    </div>
    <?php endif; ?>
</div>
