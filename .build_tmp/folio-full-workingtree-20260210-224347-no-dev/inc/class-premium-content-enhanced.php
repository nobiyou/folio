...
class folio_Premium_Content_Enhanced {
    ...
    public function vip_content_shortcode($atts, $content = '') {
        $atts = shortcode_atts(array(
            'message' => __('This content is available to VIP members only', 'folio'),
            'upgrade_text' => __('Upgrade to VIP', 'folio'),
    ...
    public function svip_content_shortcode($atts, $content = '') {
        $atts = shortcode_atts(array(
            'message' => __('This content is available to SVIP members only', 'folio'),
            'upgrade_text' => __('Upgrade to SVIP', 'folio'),
            'upgrade_url' => home_url('/user-center/membership'),
            'preview' => 'auto',
            'preview_length' => 100
        ), $atts);
    ...
    public function member_content_shortcode($atts, $content = '') {
        $atts = shortcode_atts(array(
            'level' => 'vip',
            'message' => __('This content is available to members only', 'folio'),
            'upgrade_text' => __('Become a Member', 'folio'),
            'upgrade_url' => home_url('/user-center/membership'),
            'preview' => 'auto',
            'preview_length' => 100
        ), $atts);
        ...
        <div class="membership-card membership-card-<?php echo esc_attr($level); ?> <?php echo $is_current ? 'membership-card-current' : ''; ?> <?php echo $is_highlight ? 'membership-card-highlight' : ''; ?>">
            ...
            <div class="membership-card-action">
                ...
                <button class="membership-btn membership-btn-current" disabled>
                    <span class="dashicons dashicons-yes"></span> <?php esc_html_e('Current Level', 'folio'); ?>
                </button>
                <?php else : ?>
                <a href="<?php echo esc_url(home_url('/user-center/membership')); ?>" class="membership-btn membership-btn-upgrade" data-level="<?php echo esc_attr($level); ?>">
                    ...
                    <?php echo esc_html($level === 'svip' ? __('Upgrade to SVIP', 'folio') : __('Upgrade to VIP', 'folio')); ?>
                </a>
                <?php endif; ?>
            </div>
        </div>
    ...
    public function upgrade_prompt_shortcode($atts) {
        $atts = shortcode_atts(array(
            'title' => __('Upgrade Membership for More Benefits', 'folio'),
            'description' => __('Become a VIP member to unlock all exclusive content and features.', 'folio'),
            'button_text' => __('Upgrade Now', 'folio'),
        ...
        <div class="premium-content premium-content-locked <?php echo esc_attr($level_class); ?>" data-level="<?php echo esc_attr($required_level); ?>">
            ...
            <div class="premium-content-lock">
                ...
                <div class="premium-lock-content">
                    ...
                    <p class="premium-lock-description"><?php esc_html_e('Please log in to view exclusive content', 'folio'); ?></p>
                    <div class="premium-lock-actions">
                        <a href="<?php echo esc_url(home_url('/user-center/login')); ?>" class="premium-btn premium-btn-login">
                            <span class="dashicons dashicons-admin-users"></span> <?php esc_html_e('Log In', 'folio'); ?>
                        </a>
                        <a href="<?php echo esc_url(home_url('/user-center/register')); ?>" class="premium-btn premium-btn-register">
                            <span class="dashicons dashicons-plus"></span> <?php esc_html_e('Register', 'folio'); ?>
                        </a>
                    </div>
                    <?php else : ?>
                    <p class="premium-lock-description">
                        <?php esc_html_e('Current Level:', 'folio'); ?> <?php echo esc_html($user_membership['name']); ?>
    ...
    private function get_default_features($level) {
        $features = array(
            ...
            'svip' => array(
                ...
                __('Ad-free browsing experience', 'folio'),
                __('24/7 dedicated support', 'folio'),
    ...
    public function add_premium_content_buttons() {
        ...
        <div class="folio-premium-buttons" style="display: inline-block; margin-left: 10px;">
            <button type="button" class="button" id="folio-add-vip-content" title="<?php echo esc_attr__('Insert VIP exclusive content', 'folio'); ?>">
                <span class="dashicons dashicons-lock" style="vertical-align: middle;"></span>
                <?php esc_html_e('VIP Content', 'folio'); ?>
            </button>
            <button type="button" class="button" id="folio-add-svip-content" title="<?php echo esc_attr__('Insert SVIP exclusive content', 'folio'); ?>">
                <span class="dashicons dashicons-admin-network" style="vertical-align: middle;"></span>
                <?php esc_html_e('SVIP Content', 'folio'); ?>
            </button>
            <button type="button" class="button" id="folio-add-membership-card" title="<?php echo esc_attr__('Insert membership card', 'folio'); ?>">
                <span class="dashicons dashicons-id-alt" style="vertical-align: middle;"></span>
                <?php esc_html_e('Membership Card', 'folio'); ?>
            </button>
        </div>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            document.getElementById('folio-add-vip-content')?.addEventListener('click', function() {
                const content = '[vip_content preview="auto" preview_length="100"]<?php echo esc_js(__('Enter VIP exclusive content here', 'folio')); ?>[/vip_content]';
                insertContent(content);
            });
            
            // SVIP内容按钮
            document.getElementById('folio-add-svip-content')?.addEventListener('click', function() {
                const content = '[svip_content preview="auto" preview_length="100"]<?php echo esc_js(__('Enter SVIP exclusive content here', 'folio')); ?>[/svip_content]';
                insertContent(content);
            });
            
            // 会员卡片按钮
            document.getElementById('folio-add-membership-card')?.addEventListener('click', function() {
                const content = '[membership_card level="vip" price="¥99" duration="<?php echo esc_js(__('month', 'folio')); ?>" features="<?php echo esc_js(__('exclusive content,ad-free,priority support', 'folio')); ?>" highlight="true"]';
                insertContent(content);
            });
    ...
    public function filter_article_excerpt($excerpt) {
        ...
        return sprintf(
            ...
            __('This content is members only. %s level is required to view it.', 'folio'),
    ...
    public function filter_feed_content($content) {
        ...
        return __('This content is members only. Please visit the website to view the full content.', 'folio');
    }
    */
    ...
    */
    ...
    public function get_permission_prompt($post_id, $protection_info = null) {
        ...
        <div class="folio-permission-prompt <?php echo esc_attr($level_class); ?>">
            ...
            <div class="folio-prompt-content">
                <h4 class="folio-prompt-title"><?php echo esc_html(sprintf(__('%s members only content', 'folio'), $level_name)); ?></h4>
                
                <?php if (!$user_id) : ?>
                <p class="folio-prompt-description"><?php esc_html_e('Please log in to view exclusive content', 'folio'); ?></p>
                ...
                <div class="folio-prompt-actions">
                    <a href="<?php echo esc_url(home_url('/user-center/membership')); ?>" class="folio-btn folio-btn-upgrade">
                        <span class="dashicons dashicons-star-filled"></span>
                        <?php echo esc_html(sprintf(__('Upgrade to %s', 'folio'), $level_name)); ?>
                    </a>
                </div>
                <?php endif; ?>
            </div>
    ...
    public function add_premium_content_meta_box() {
        add_meta_box(
            ...
            __('Member-only Content Settings', 'folio'),
    ...
    public function render_premium_content_meta_box($post) {
        ...
        <div class="folio-premium-settings">
            <p>
                <label>
                    ...
                    <strong><?php esc_html_e('Set as member-only post', 'folio'); ?></strong>
                </label>
            </p>
            ...
            <div id="folio_premium_options" style="<?php echo $protection_info['is_protected'] ? '' : 'display:none;'; ?>">
                <!-- 会员等级设置 -->
                <p>
                    <label for="folio_required_level"><strong><?php esc_html_e('Required Level:', 'folio'); ?></strong></label><br>
                    <select name="folio_required_level" id="folio_required_level" style="width: 100%;">
                        <option value="vip" <?php selected($protection_info['required_level'], 'vip'); ?>><?php esc_html_e('VIP Member', 'folio'); ?></option>
                        <option value="svip" <?php selected($protection_info['required_level'], 'svip'); ?>><?php esc_html_e('SVIP Member', 'folio'); ?></option>
                    </select>
                </p>
                
                <!-- 预览模式设置 -->
                <p>
                    <label for="folio_preview_mode"><strong><?php esc_html_e('Preview Mode:', 'folio'); ?></strong></label><br>
                    <select name="folio_preview_mode" id="folio_preview_mode" style="width: 100%;">
                        <option value="<?php echo self::PREVIEW_AUTO; ?>" <?php selected($protection_info['preview_mode'], self::PREVIEW_AUTO); ?>><?php esc_html_e('Auto Preview', 'folio'); ?></option>
                        <option value="<?php echo self::PREVIEW_PERCENTAGE; ?>" <?php selected($protection_info['preview_mode'], self::PREVIEW_PERCENTAGE); ?>><?php esc_html_e('Percentage Preview', 'folio'); ?></option>
                        <option value="<?php echo self::PREVIEW_CUSTOM; ?>" <?php selected($protection_info['preview_mode'], self::PREVIEW_CUSTOM); ?>><?php esc_html_e('Custom Preview', 'folio'); ?></option>
                        <option value="<?php echo self::PREVIEW_NONE; ?>" <?php selected($protection_info['preview_mode'], self::PREVIEW_NONE); ?>><?php esc_html_e('No Preview', 'folio'); ?></option>
                    </select>
                </p>
                ...
                <div id="folio_auto_preview_settings" style="<?php echo $protection_info['preview_mode'] === self::PREVIEW_AUTO ? '' : 'display:none;'; ?>">
                    <p>
                        <label for="folio_preview_length"><?php esc_html_e('Preview Length (characters):', 'folio'); ?></label><br>
                        ...
                        <small><?php esc_html_e('Recommended: 50-500 characters', 'folio'); ?></small>
                    </p>
                </div>
                ...
                <div id="folio_percentage_preview_settings" style="<?php echo $protection_info['preview_mode'] === self::PREVIEW_PERCENTAGE ? '' : 'display:none;'; ?>">
                    <p>
                        <label for="folio_preview_percentage"><?php esc_html_e('Preview Percentage:', 'folio'); ?></label><br>
                        ...
                        <br><small><?php esc_html_e('Recommended: 20%-50%', 'folio'); ?></small>
                    </p>
                </div>
                ...
                <div id="folio_custom_preview_settings" style="<?php echo $protection_info['preview_mode'] === self::PREVIEW_CUSTOM ? '' : 'display:none;'; ?>">
                    <p>
                        <label for="folio_preview_custom"><?php esc_html_e('Custom Preview Content:', 'folio'); ?></label><br>
                        <textarea name="folio_preview_custom" id="folio_preview_custom" 
                                  rows="4" style="width: 100%;" placeholder="<?php echo esc_attr__('Enter custom preview text...', 'folio'); ?>"><?php echo esc_textarea($protection_info['preview_custom']); ?></textarea>
                        <small><?php esc_html_e('Supports HTML tags', 'folio'); ?></small>
                    </p>
                </div>
                
                <!-- 高级设置 -->
                <details style="margin-top: 15px;">
                    <summary style="cursor: pointer; font-weight: bold;"><?php esc_html_e('Advanced Settings', 'folio'); ?></summary>
                    <div style="margin-top: 10px;">
                        <p>
                            <label>
                                <input type="checkbox" name="folio_seo_visible" value="1" <?php checked($protection_info['seo_visible'], true); ?>>
                                <?php esc_html_e('Search engine visible (allow indexing preview content)', 'folio'); ?>
                            </label>
                        </p>
                        <p>
                            <label>
                                <input type="checkbox" name="folio_rss_include" value="1" <?php checked($protection_info['rss_include'], true); ?>>
                                <?php esc_html_e('Include in RSS (show preview content in RSS)', 'folio'); ?>
                            </label>
                        </p>
                    </div>
                </details>
        ...
        <div class="folio-shortcode-help" style="margin-top: 15px; padding: 10px; background: #f0f0f1; border-radius: 4px;">
            <h4 style="margin: 0 0 10px 0;"><?php esc_html_e('Quick Shortcode Insert:', 'folio'); ?></h4>
            <p style="margin: 5px 0; font-size: 12px;"><code>[vip_content]VIP exclusive content[/vip_content]</code></p>
            <p style="margin: 5px 0; font-size: 12px;"><code>[membership_card level="vip" price="¥99"]</code></p>
            <p style="margin: 5px 0; font-size: 12px;"><code>[upgrade_prompt title="Upgrade prompt"]</code></p>
        </div>
        <?php
    }
    ...
    public function ajax_unlock_content() {
        ...
        if ($this->can_access_content($user_membership, $required_level)) {
            wp_send_json_success(array('message' => __('Content unlocked', 'folio')));
        } else {
    ...
    public static function render_membership_badge($post_id, $context = 'list') {
        ...
        if ($can_access) {
            ...
            $badge_text = sprintf(__('%s Unlocked', 'folio'), $level_name);
        } else {
            $badge_class .= ' folio-premium-badge-locked';
            $badge_text = sprintf(__('Requires %s', 'folio'), $level_name);
        }
        
        return sprintf(
            '<span class="%s" title="%s">%s</span>',
            esc_attr($badge_class),
            esc_attr($can_access ? __('You can view this content', 'folio') : sprintf(__('Requires %s level to view', 'folio'), $level_name)),
            esc_html($badge_text)
        );
    }
}
