        ...
        if (is_front_page() || is_home()) {
            ...
            if (!empty($ai_keywords)) {
                ...
                if ($categories && !is_wp_error($categories)) {
                    $term_names = wp_list_pluck($categories, 'name');
            ...
            $description = $term->description ?: sprintf(__('%s category archive', 'folio'), $term->name);

        } elseif (is_home() || is_archive()) {
            // 文章归档页
            $keywords = __('blog,posts', 'folio');
            $description = __('Browse latest posts', 'folio');

        } elseif (is_search()) {
            // 搜索页
            $keywords = get_search_query();
            $description = sprintf(__('Search results for "%s"', 'folio'), get_search_query());

        } elseif (is_404()) {
            // 404 页面
            $description = __('Page not found', 'folio');
        }
