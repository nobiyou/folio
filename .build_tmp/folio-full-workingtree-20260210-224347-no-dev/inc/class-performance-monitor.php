...
class folio_Performance_Monitor {
    ...
    public function display_performance_bar() {
        ...
        <div id="folio-performance-bar">
            <div class="performance-metrics">
                <div title="<?php echo esc_attr__('Page Load Time', 'folio'); ?>" class="performance-metric <?php echo $performance['load_time'] > 3 ? 'performance-metric-critical' : ($performance['load_time'] > 2 ? 'performance-metric-warning' : 'performance-metric-good'); ?>">
                ...
                <div title="<?php echo esc_attr__('Memory Usage', 'folio'); ?>" class="performance-metric <?php echo $performance['memory_usage'] > 64*1024*1024 ? 'performance-metric-warning' : 'performance-metric-good'; ?>">
                ...
                <div title="<?php echo esc_attr__('Database Queries', 'folio'); ?>" class="performance-metric <?php echo $performance['db_queries'] > 50 ? 'performance-metric-critical' : ($performance['db_queries'] > 30 ? 'performance-metric-warning' : 'performance-metric-good'); ?>">
                ...
                <div title="<?php echo esc_attr__('Query Time', 'folio'); ?>" class="performance-secondary">
                ...
                <div title="<?php echo esc_attr__('Peak Memory', 'folio'); ?>" class="performance-secondary">
                ...
                <div title="<?php echo esc_attr__('Page Type', 'folio'); ?>" class="performance-page-type">
                ...
                <div title="<?php echo esc_attr__('Slow Query Warning', 'folio'); ?>" class="performance-metric performance-metric-critical">
                ...
                if ($critical_count > 0) : ?>
                <div title="<?php echo esc_attr(sprintf(__('%d critical optimization suggestions', 'folio'), $critical_count)); ?>" class="performance-metric performance-metric-critical">
            ...
            <div class="performance-controls">
                <button onclick="folioTogglePerformanceDetails()" class="performance-btn performance-btn-details" title="<?php echo esc_attr__('View details and optimization suggestions', 'folio'); ?>">
                    <?php esc_html_e('Details', 'folio'); ?>
                </button>
                <button onclick="folioHidePerformanceBar()" class="performance-btn performance-btn-close" title="<?php echo esc_attr__('Hide toolbar (Ctrl+Shift+P to show again)', 'folio'); ?>">
                    ✕
                </button>
            </div>
        </div>
        ...
        <div id="folio-performance-details">
            <h4 class="performance-details-header"><?php esc_html_e('Performance Details', 'folio'); ?></h4>
            
            <div class="performance-details-section">
                <strong><?php esc_html_e('Page Info:', 'folio'); ?></strong><br>
                URL: <?php echo esc_html($_SERVER['REQUEST_URI'] ?? ''); ?><br>
                <?php esc_html_e('Page Type:', 'folio'); ?> <?php echo esc_html($this->get_page_type()); ?><br>
                <?php esc_html_e('Device Type:', 'folio'); ?> <?php echo wp_is_mobile() ? esc_html__('Mobile Device', 'folio') : esc_html__('Desktop Device', 'folio'); ?><br>
                <?php esc_html_e('User Agent:', 'folio'); ?> <?php echo esc_html(substr($_SERVER['HTTP_USER_AGENT'] ?? '', 0, 50)); ?>...
            </div>

            <div class="performance-details-section">
                <strong><?php esc_html_e('Performance Metrics:', 'folio'); ?></strong><br>
                <?php esc_html_e('Load Time:', 'folio'); ?> <?php echo number_format($performance['load_time'], 3); ?>s<br>
                <?php esc_html_e('Memory Usage:', 'folio'); ?> <?php echo $this->format_bytes($performance['memory_usage']); ?><br>
                <?php esc_html_e('Peak Memory:', 'folio'); ?> <?php echo $this->format_bytes($performance['memory_peak']); ?><br>
                <?php echo esc_html(sprintf(__('%d database queries', 'folio'), $performance['db_queries'])); ?><br>
                <?php esc_html_e('Total Query Time:', 'folio'); ?> <?php echo number_format($performance['db_time'], 3); ?>s
            </div>

            <?php if (!empty($performance['slow_queries_list'])) : ?>
            <div class="performance-details-section">
                <strong style="color: #ff6b6b;"><?php esc_html_e('Slow Queries (>0.1s):', 'folio'); ?></strong><br>
            ...
            <div class="performance-details-section">
                <strong style="color: #ffa726;"><?php esc_html_e('Optimization Suggestions:', 'folio'); ?></strong><br>
                <?php foreach ($suggestions as $suggestion) : ?>
                <div class="folio-optimization-suggestion <?php echo $suggestion['type']; ?>">
                    <div class="type">
                        <?php echo $suggestion['type'] === 'critical' ? esc_html__('[Critical]', 'folio') : esc_html__('[Warning]', 'folio'); ?>
                    </div>
            ...
            <div class="performance-details-section">
                <strong><?php esc_html_e('Environment Info:', 'folio'); ?></strong><br>
                PHP: <?php echo PHP_VERSION; ?><br>
                WordPress: <?php echo get_bloginfo('version'); ?><br>
                <?php esc_html_e('Theme:', 'folio'); ?> <?php echo wp_get_theme()->get('Name'); ?><br>
                <?php echo esc_html(sprintf(__('%d active plugins', 'folio'), count(get_option('active_plugins', array())))); ?>
            </div>

            <div class="folio-performance-tip">
                <div class="title"><?php esc_html_e('Tips:', 'folio'); ?></div>
                <div class="content">
                    • <?php esc_html_e('Click × to hide the toolbar', 'folio'); ?><br>
                    • <?php esc_html_e('Press', 'folio'); ?> <kbd>Ctrl+Shift+P</kbd> <?php esc_html_e('to toggle show/hide', 'folio'); ?><br>
                    • <kbd>Ctrl+<?php esc_html_e('Double Click', 'folio'); ?></kbd> <?php esc_html_e('to show it again', 'folio'); ?><br>
                    • <?php esc_html_e('Green = good, yellow = warning, red = needs optimization', 'folio'); ?>
                </div>
            </div>
        </div>
    ...
    public function ajax_clear_performance_logs() {
        ...
        wp_send_json_success(array('message' => __('Performance logs have been cleared', 'folio')));
    }
    ...
    private function get_optimization_suggestions($performance) {
        ...
        if ($performance['load_time'] > 3) {
            $suggestions[] = array(
                ...
                'message' => sprintf(
                    ...
                    __('Page load time is too long (>%ss). Optimization is recommended.', 'folio'),
                    number_format($performance['load_time'], 2)
                )
            );
        } elseif ($performance['load_time'] > 2) {
            $suggestions[] = array(
                'type' => 'warning',
                'message' => __('Page load time is relatively long and can be further optimized', 'folio')
            );
        }
        ...
        if ($performance['memory_usage'] > 64 * 1024 * 1024) { // 64MB
            $suggestions[] = array(
                'type' => 'warning',
                'message' => sprintf(
                    /* translators: %s: memory usage size */
                    __('Memory usage is high (%s). Check plugins and theme.', 'folio'),
                    $this->format_bytes($performance['memory_usage'])
                )
            );
        }
        ...
        if ($performance['db_queries'] > 50) {
            $suggestions[] = array(
                ...
                'message' => sprintf(
                    ...
                    __('Too many database queries (%d). Cache usage is recommended.', 'folio'),
                    $performance['db_queries']
                )
            );
        } elseif ($performance['db_queries'] > 30) {
            $suggestions[] = array(
                'type' => 'warning',
                'message' => __('Database query count is high. Consider query optimization or enabling cache.', 'folio')
            );
        }
        ...
        if ($performance['slow_queries'] > 0) {
            $suggestions[] = array(
                ...
                'message' => sprintf(
                    ...
                    __('Detected %d slow queries. Database query optimization is required.', 'folio'),
