...
class folio_Security_Protection_Manager {
    ...
    public function enhanced_feed_protection($content) {
        ...
        if (!$protection_info['rss_include']) {
            ...
            return __('This content is members only. Please visit the site to view the full content.', 'folio');
        }
    ...
    public function enhanced_rest_protection($response, $post, $request) {
        ...
        if (!$can_access) {
            ...
            $data['content']['rendered'] = __('This content is members only and requires the corresponding membership level.', 'folio');
    ...
    public function search_engine_content_filter($content) {
        ...
        if ($protection_info['seo_visible']) {
            ...
            return $preview . "\n\n[" . __('Members-only content', 'folio') . "]";
        }

        return __('This content is members only. Please log in and ensure you have the required permission level.', 'folio');
    }
        ...
        foreach ($nets as $net) {
            if ($this->ip_in_network($ip, $net->ip_net)) {
                return array(
                    ...
                    'spider_name' => $net->spider_name ?: 'Unknown Spider'
                );
            }
        }
    ...
    public function get_spider_info_by_user_agent($user_agent) {
        ...
        $spider_patterns = array(
            'googlebot' => array('spider_id' => 12, 'spider_name' => 'Google Crawler'),
            'bingbot' => array('spider_id' => 16, 'spider_name' => 'Bing Crawler'),
            'slurp' => array('spider_id' => 15, 'spider_name' => 'Yahoo Crawler'),
            'baiduspider' => array('spider_id' => 13, 'spider_name' => 'Baidu Crawler'),
            'sogou' => array('spider_id' => 17, 'spider_name' => 'Sogou Crawler'),
            'yandexbot' => array('spider_id' => 18, 'spider_name' => 'Yandex Crawler'),
            '360spider' => array('spider_id' => 11, 'spider_name' => '360 Crawler'),
            'bytespider' => array('spider_id' => 14, 'spider_name' => 'Toutiao Crawler'),
            'yisouspider' => array('spider_id' => 19, 'spider_name' => 'Shenma Crawler'),
        );
    ...
    private function send_blocked_response() {
        status_header(429);
        wp_die(
            __('Too many requests. Please try again later. If you believe this is an error, contact the site administrator.', 'folio'),
            __('Access Restricted', 'folio'),
            array('response' => 429)
        );
    }
    ...
    private function get_blocked_message() {
        return '<div class="folio-blocked-message">
            <h3>' . esc_html__('Access Restricted', 'folio') . '</h3>
            <p>' . esc_html__('Suspicious access behavior was detected. Your access has been temporarily limited.', 'folio') . '</p>
            <p>' . esc_html__('If you believe this is an error, please contact the site administrator.', 'folio') . '</p>
        </div>';
    }
    ...
    public function import_spider_nets($spider_data) {
        ...
        foreach ($spider_data as $spider_group) {
            ...
            $spider_name = isset($spider_names[$spider_id]) ? $spider_names[$spider_id] : 'Unknown Spider';
    ...
    public function analyze_logs_for_spiders($days = 7, $min_count = 3) {
        ...
        $spider_patterns = array(
            'googlebot' => array('spider_id' => 12, 'spider_name' => 'Google Crawler'),
            'bingbot' => array('spider_id' => 16, 'spider_name' => 'Bing Crawler'),
            'msnbot' => array('spider_id' => 16, 'spider_name' => 'Bing Crawler'),
            'slurp' => array('spider_id' => 15, 'spider_name' => 'Yahoo Crawler'),
            'baiduspider' => array('spider_id' => 13, 'spider_name' => 'Baidu Crawler'),
            'sogou' => array('spider_id' => 17, 'spider_name' => 'Sogou Crawler'),
            'yandexbot' => array('spider_id' => 18, 'spider_name' => 'Yandex Crawler'),
            '360spider' => array('spider_id' => 11, 'spider_name' => '360 Crawler'),
            '360' => array('spider_id' => 11, 'spider_name' => '360 Crawler'),
            'bytespider' => array('spider_id' => 14, 'spider_name' => 'Toutiao Crawler'),
            'yisouspider' => array('spider_id' => 19, 'spider_name' => 'Shenma Crawler'),
            'yisou' => array('spider_id' => 19, 'spider_name' => 'Shenma Crawler'),
            'semrushbot' => array('spider_id' => 20, 'spider_name' => 'SEMrush Crawler'),
            'ahrefsbot' => array('spider_id' => 21, 'spider_name' => 'Ahrefs Crawler'),
            'majestic' => array('spider_id' => 22, 'spider_name' => 'Majestic Crawler'),
            'dotbot' => array('spider_id' => 23, 'spider_name' => 'DotBot Crawler'),
            'mj12bot' => array('spider_id' => 24, 'spider_name' => 'MJ12Bot Crawler'),
            'petalbot' => array('spider_id' => 25, 'spider_name' => 'PetalBot Crawler'),
            'applebot' => array('spider_id' => 26, 'spider_name' => 'AppleBot Crawler'),
            'facebookexternalhit' => array('spider_id' => 27, 'spider_name' => 'Facebook Crawler'),
            'twitterbot' => array('spider_id' => 28, 'spider_name' => 'TwitterBot Crawler'),
            'linkedinbot' => array('spider_id' => 29, 'spider_name' => 'LinkedInBot Crawler'),
            'whatsapp' => array('spider_id' => 30, 'spider_name' => 'WhatsApp Crawler'),
            'telegrambot' => array('spider_id' => 31, 'spider_name' => 'TelegramBot Crawler'),
            'discordbot' => array('spider_id' => 32, 'spider_name' => 'DiscordBot Crawler'),
            'pinterest' => array('spider_id' => 33, 'spider_name' => 'Pinterest Crawler'),
            'redditbot' => array('spider_id' => 34, 'spider_name' => 'RedditBot Crawler'),
            'duckduckbot' => array('spider_id' => 35, 'spider_name' => 'DuckDuckGo Crawler'),
            'exabot' => array('spider_id' => 36, 'spider_name' => 'ExaBot Crawler'),
        );
        ...
        foreach ($logs as $log) {
            ...
            // 如果没有匹配到已知模式，但包含常见的爬虫关键词，归类为Generic Crawler
            if (!$spider_info) {
                ...
                foreach ($crawler_keywords as $keyword) {
                    if (strpos($user_agent_lower, $keyword) !== false) {
                        $spider_info = array('spider_id' => 99, 'spider_name' => 'Generic Crawler');
                        break;
                    }
                }
            }
