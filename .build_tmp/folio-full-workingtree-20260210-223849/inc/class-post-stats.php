...
class folio_Post_Stats {
    ...
    public function __construct() {
        ...
        // 在后台显示统计列
        add_filter('manage_posts_columns', array($this, 'add_stats_columns'));
        add_action('manage_posts_custom_column', array($this, 'display_stats_columns'), 10, 2);
        add_filter('manage_edit-post_sortable_columns', array($this, 'sortable_stats_columns'));
    ...
    public function ajax_like_post() {
        ...
        if (!$post_id || get_post_type($post_id) !== 'post') {
            wp_send_json_error(array('message' => __('Invalid post', 'folio')));
        }

        // 检查是否已点赞（通过 cookie）
        $cookie_name = 'folio_liked_' . $post_id;
        if (isset($_COOKIE[$cookie_name])) {
            wp_send_json_error(array('message' => __('You have already liked this post', 'folio')));
        }
        ...
        wp_send_json_success(array(
            'likes' => $new_likes,
            'message' => __('Thanks for your like!', 'folio')
        ));
    }
    ...
    public function ajax_favorite_post() {
        ...
        if (!is_user_logged_in()) {
            wp_send_json_error(array(
                'message' => __('Please sign in first', 'folio'),
                'need_login' => true
            ));
        }
        ...
        if ($action_type === 'remove') {
            ...
            if ($result) {
                wp_send_json_success(array(
                    ...
                    'is_favorited' => false,
                    'message' => __('Removed from favorites', 'folio')
                ));
            } else {
                wp_send_json_error(array('message' => __('Failed to remove favorite', 'folio')));
            }
        } else {
            // 添加收藏
            if (self::has_favorited($post_id)) {
                wp_send_json_error(array('message' => __('You have already favorited this post', 'folio')));
            }
            ...
            if ($result) {
                wp_send_json_success(array(
                    'favorites' => self::get_favorites($post_id),
                    'is_favorited' => true,
                    'message' => __('Added to favorites!', 'folio')
                ));
            } else {
                wp_send_json_error(array('message' => __('Failed to add favorite', 'folio')));
            }
        }
    }
    ...
    public function render_user_favorites($atts) {
        if (!is_user_logged_in()) {
            return '<p>' . __('Please sign in to view favorites', 'folio') . '</p>';
        }
        ...
        if (empty($favorites)) {
            return '<p>' . __('No favorite posts yet', 'folio') . '</p>';
        }
    ...
    public function add_stats_columns($columns) {
        ...
        foreach ($columns as $key => $value) {
            ...
            if ($key === 'date') {
                $new_columns['views'] = __('Views', 'folio');
                $new_columns['likes'] = __('Likes', 'folio');
                $new_columns['follow_num'] = __('Favorites', 'folio');
            }
        }
