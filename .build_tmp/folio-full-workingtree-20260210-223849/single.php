...
while (have_posts()) : the_post();
    ...
    $image_count = function_exists('folio_get_post_image_count')
        ? folio_get_post_image_count(get_the_ID(), true)
        : 0;
?>
    ...
    <div class="md:col-span-8 lg:col-span-9">        
        ...
        <div class="flex flex-wrap items-center gap-4 mt-8 py-4 text-xs text-gray-500">
            ...
            <div class="flex items-center gap-1.5">
                ...
                <span><?php echo esc_html(folio_format_post_date(get_the_ID())); ?></span>
            </div>
...
<div class="grid grid-cols-1 lg:grid-cols-12 gap-8">
    <!-- 文章内容 -->
    <main class="max-w-none lg:col-span-8">
        <div class="max-w-4xl">
            <?php the_content(); ?>
        </div>
    </main>
        ...
        <article class="related-post-card group">
            <a href="<?php echo esc_url(get_permalink($related_post)); ?>" class="block">
                ...
                <div class="related-post-thumbnail relative overflow-hidden rounded-lg mb-4 aspect-[3/4]">
                    ...
                    $related_thumb_html = folio_get_post_thumbnail_with_fallback($related_post->ID, 'medium', array(
                        'class' => $related_image_class,
                    ));
                    if ($related_thumb_html) :
                    ?>
                        <?php echo $related_thumb_html; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?>
                    <?php else : ?>
    ...
    <a href="<?php echo esc_url(get_permalink($nav_post)); ?>" class="nav-card group block rounded-lg overflow-hidden">
        <div class="flex flex-col md:flex-row h-auto md:h-64">
            <div class="nav-card-image-wrapper md:w-2/5 relative overflow-hidden">
                ...
                $nav_thumb_html = folio_get_post_thumbnail_with_fallback($nav_post->ID, 'post-hero', array(
                    'class' => $nav_image_class,
                ));
                if ($nav_thumb_html) :
                ?>
                    <?php echo $nav_thumb_html; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?>
                <?php else : ?>
