...
?>
<article id="post-<?php the_ID(); ?>" <?php post_class('post-card relative group aspect-[3/4] rounded overflow-hidden shadow-sm hover:shadow-lg transition'); ?> itemscope itemtype="https://schema.org/Article">
    ...
    if ($thumbnail_id) {
        ...
        $thumbnail_html = get_the_post_thumbnail(get_the_ID(), 'post-card', array(
            'class'   => $image_classes,
            'alt'     => get_the_title(),
            'loading' => 'lazy',
        ));
    ...
    <!-- 链接覆盖 -->
    <a href="<?php the_permalink(); ?>" class="absolute inset-0 z-20" aria-label="<?php echo esc_attr(sprintf(__('View %s', 'folio'), get_the_title())); ?>" itemprop="url"></a>
</article>
