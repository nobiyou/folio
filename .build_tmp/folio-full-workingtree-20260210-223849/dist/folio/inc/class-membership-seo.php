...
class folio_Membership_SEO {
    ...
    public function output_membership_meta_tags() {
        ...
        if ($protection_info['seo_visible']) {
            ...
            // 对于SEO不可见的内容，使用通用描述
            echo '<meta name="description" content="' . esc_attr__('This content is for members only. Please log in to view.', 'folio') . '">' . "\n";
    ...
    public function modify_title_for_protected_content($title_parts) {
        ...
        if (!$can_access && $protection_info['seo_visible']) {
            $level_name = $protection_info['required_level'] === 'svip' ? 'SVIP' : 'VIP';
            $title_parts['title'] = $title_parts['title'] . ' [' . sprintf(__('%s Exclusive', 'folio'), $level_name) . ']';
        }
    ...
    public function output_open_graph_tags() {
        ...
        if (!$can_access && $protection_info['seo_visible']) {
            $level_name = $protection_info['required_level'] === 'svip' ? 'SVIP' : 'VIP';
            $og_title .= ' [' . sprintf(__('%s Exclusive', 'folio'), $level_name) . ']';
        }
        ...
        if ($protection_info['seo_visible']) {
            ...
            $og_description = __('This content is for members only. Please log in to view the full content.', 'folio');
        }
    ...
    public function enhance_article_schema($schema, $post_id) {
        ...
        // 添加访问要求信息
        $schema['conditionsOfAccess'] = sprintf(__('Requires %s membership level', 'folio'), $level_name);
        ...
        $schema['isPartOf'] = array(
            '@type' => 'CreativeWork',
            'name' => get_bloginfo('name') . ' ' . __('Member Content', 'folio'),
            'description' => __('Exclusive content that requires membership subscription', 'folio')
        );
        ...
        if ($protection_info['seo_visible']) {
            ...
            if ($preview) {
                return wpautop($preview) . "\n\n" . 
                       '<p><em>' . sprintf(
                           __('This content is for members only. %s level is required to view full content.', 'folio'),
                           ($protection_info['required_level'] === 'svip' ? 'SVIP' : 'VIP')
                       ) . '</em></p>';
            }
        }

        return '<p>' . esc_html__('This content is for members only. Please log in to view.', 'folio') . '</p>';
    }
    ...
    public function filter_excerpt_for_crawlers($excerpt) {
        ...
        $level_name = $protection_info['required_level'] === 'svip' ? 'SVIP' : 'VIP';
        return sprintf(__('This content is for %s members only.', 'folio'), $level_name) . ($excerpt ? ' ' . $excerpt : '');
    }
    ...
    public function prevent_content_leakage() {
        ...
        if (isset($_GET['preview']) || isset($_GET['debug']) || isset($_GET['raw'])) {
            if (!current_user_can('manage_options')) {
                wp_die(__('Access denied', 'folio'), __('Insufficient permissions', 'folio'), array('response' => 403));
            }
        }
    }
        ...
        if ($protection_info['preview_mode'] !== 'none') {
            ...
            if ($preview) {
                ...
                return wp_trim_words(wp_strip_all_tags($preview), 25, '...') .
                       ' [' . sprintf(__('Requires %s membership to view full content', 'folio'), $level_name) . ']';
            }
        }

        $level_name = $protection_info['required_level'] === 'svip' ? 'SVIP' : 'VIP';
        return sprintf(
            __('This content is for %s members only and includes high-quality premium materials. Upgrade membership to unlock full access.', 'folio'),
            $level_name
        );
    }
