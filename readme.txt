...
一个现代化的作品展示主题，专为设计师和创意工作者打造，基于 WordPress 默认文章类型。

== 主题说明 ==

Folio 是面向设计师与创作者的 WordPress 主题，采用六边形卡片与现代化排版，支持博客/作品展示、用户中心、会员体系及丰富后台配置。

* 前台：响应式布局、深色/浅色模式、作品分类（基于文章分类）、用户中心（登录/注册/收藏/个人资料）、会员等级（普通/VIP/SVIP）、文章统计（浏览/点赞/收藏）、社交链接、多语言
* 后台：主题设置（外观、社交、SEO、性能）、安全与访问日志、性能与缓存管理、会员与通知管理、AI 辅助内容等
* 技术：内置 SEO、性能优化、注册邮箱验证码、用户封禁与黑名单、国际化（.pot/.po）

== Description ==

Folio 是一个简洁、现代的 WordPress 作品展示主题，完全基于标准文章类型，具有以下特点：
...
* 内容模型使用说明：docs/CONTENT-MODEL-GUIDE.md（介绍如何使用默认文章类型构建作品/博客，并提供迁移建议）
...
激活主题后，可直接在 WordPress 后台「文章 > 新建文章」中发布内容，并使用分类/标签区分不同类型的作品或项目。
