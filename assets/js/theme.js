...
(function() {
    ...
    function initKeyboardNavigation() {
        // Add keyboard support for portfolio cards
        const portfolioCards = document.querySelectorAll('.portfolio-card, article[class*="portfolio"]');
    ...
    function showShareModal() {
        ...
        modal.innerHTML = `
            <div style="background:#fff;border-radius:12px;padding:24px;max-width:320px;margin:16px;box-shadow:0 20px 25px -5px rgba(0,0,0,0.1);">
                <h3 style="font-size:18px;font-weight:700;margin-bottom:16px;">分享到</h3>
                ...
                <div style="display:flex;align-items:center;gap:8px;background:#f3f4f6;border-radius:8px;padding:8px 12px;">
                    ...
                    <button onclick="document.getElementById('mpb-share-url').select();navigator.clipboard.writeText('${url}');" style="font-size:11px;font-weight:700;text-transform:uppercase;color:#666;cursor:pointer;border:none;background:none;">复制</button>
                </div>
                <button onclick="this.closest('.mpb-share-modal').remove()" style="margin-top:16px;width:100%;padding:8px;font-size:12px;font-weight:700;text-transform:uppercase;color:#666;cursor:pointer;border:none;background:none;">关闭</button>
            </div>
        `;
        ...
        likeBtn.addEventListener('click', function() {
            ...
            .then(data => {
                if (data.success) {
                    ...
                    showNotification(data.data.message || '点赞失败');
                }
            })
            .catch(function() {
            ...
            .then(data => {
                if (data.success) {
                    ...
                    if (data.data && data.data.need_login) {
                        ...
                        showNotification(data.data && data.data.message ? data.data.message : '操作失败');
                    }
                }
            })
