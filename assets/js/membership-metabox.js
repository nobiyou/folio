...
(function($) {
    ...
    function updateRangeValue($slider) {
        ...
        if ($valueSpan.length) {
            const unit = $slider.attr('id').includes('length') ? ' 字符' : '%';
    ...
    function generatePreview() {
        ...
        .done(function(response) {
            if (response.success) {
                ...
                // 显示统计信息
                const stats = `原文长度: ${response.data.original_length} 字符，预览长度: ${response.data.preview_length} 字符`;
                $result.find('h4').html('预览效果 <small>(' + stats + ')</small>');
            } else {
        ...
        .always(function() {
            $btn.prop('disabled', false).html('<span class="dashicons dashicons-visibility"></span> 生成预览效果');
        });
    }
    ...
    function loadRecentPosts() {
        ...
        $list.html('<div class="folio-loading">加载中...</div>');
        ...
        .done(function(response) {
            if (response.success) {
                ...
                $list.html('<div class="folio-error">加载失败</div>');
            }
        })
        .fail(function() {
            $list.html('<div class="folio-error">加载失败</div>');
            ...
            case 'set_preview_mode':
                html = `
                    ...
                    <select name="bulk_preview_mode" class="folio-select">
                        <option value="auto">自动预览</option>
                        <option value="percentage">百分比预览</option>
                        <option value="custom">自定义预览</option>
                        <option value="none">无预览</option>
                    </select>
                    <div style="margin-top:10px;">
                        <label>预览长度（自动模式）：</label>
                        <input type="number" name="bulk_preview_length" value="200" min="50" max="1000" class="folio-select">
                    </div>
                    <div style="margin-top:10px;">
                        <label>预览比例（百分比模式）：</label>
    ...
    function executeBulkAction() {
        ...
        if (!action) {
            showNotice('error', '请选择操作类型');
            return;
        }
        
        if (selectedPosts.length === 0) {
            showNotice('error', '请选择要操作的文章');
            return;
        }
        ...
        .always(function() {
            $btn.prop('disabled', false).text('执行操作');
        });
    }
    ...
    window.folioSetPreset = function(type) {
        ...
        showNotice('success', '已应用' + type + '保护预设');
    };
