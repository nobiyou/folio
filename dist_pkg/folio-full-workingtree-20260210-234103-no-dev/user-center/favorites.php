...
<div class="favorites-page">
    <div class="flex items-center justify-between mb-6">
        <h1 class="text-2xl font-bold"><?php esc_html_e('My Favorites', 'folio'); ?></h1>
        <span class="text-sm">
            <?php printf(esc_html__('%d works in total', 'folio'), $total); ?>
        </span>
    </div>
        ...
        <div class="favorite-card">
            <div class="relative group">
                <a href="<?php echo get_permalink($post_id); ?>" class="block">
                    <?php
                    echo folio_render_thumbnail_block($post_id, 'medium_large', array(
                        'wrapper_class' => 'favorite-image',
                        'image_class' => 'w-full h-48 object-cover rounded-lg',
                        'placeholder_class' => 'w-full h-48 rounded-lg bg-gradient-to-br from-gray-200 to-gray-300',
                    ));
                    ?>
                ...
                <button class="remove-favorite absolute top-2 right-2 w-8 h-8 bg-red-500 text-white rounded-full opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex items-center justify-center hover:bg-red-600" 
                        ...
                        title="<?php esc_attr_e('Remove Favorite', 'folio'); ?>">
    ...
    <div class="empty-state text-center py-16">
        ...
        <h3 class="empty-state-title text-xl font-medium mb-4"><?php esc_html_e('No favorites yet', 'folio'); ?></h3>
        <p class="empty-state-text mb-6"><?php esc_html_e('Explore great works and click the star icon to add favorites.', 'folio'); ?></p>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    document.querySelectorAll('.remove-favorite').forEach(function(btn) {
        btn.addEventListener('click', function(e) {
            ...
            if (!confirm('<?php echo esc_js(__('Are you sure you want to remove this favorite?', 'folio')); ?>')) {
                return;
            }
            ...
            .then(data => {
                if (data.success) {
                    ...
                    alert(data.data.message || '<?php echo esc_js(__('Operation failed', 'folio')); ?>');
                }
            })
            .catch(() => {
                alert('<?php echo esc_js(__('Network error, please try again later', 'folio')); ?>');
            });
        });
    });
});
