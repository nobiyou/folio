...
?>
    ...
    <?php
    $current_frontend_locale = function_exists('folio_get_current_frontend_locale') ? folio_get_current_frontend_locale() : get_locale();
    $is_zh = ($current_frontend_locale === 'zh_CN');
    $lang_target = $is_zh ? 'en_US' : 'zh_CN';
    $lang_url = function_exists('folio_get_language_switch_url') ? folio_get_language_switch_url($lang_target) : '#';
    $lang_label = $is_zh ? '&#20013;' : 'EN';
    ?>

    <a class="floating-lang-switch"
       href="<?php echo esc_url($lang_url); ?>"
       aria-label="<?php esc_attr_e('Language Switch', 'folio'); ?>">
        <?php echo $lang_label; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?>
    </a>
