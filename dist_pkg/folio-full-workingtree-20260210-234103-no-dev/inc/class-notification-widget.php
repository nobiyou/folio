...
class folio_Notification_Widget extends WP_Widget {
    ...
    public function __construct() {
        parent::__construct(
            ...
            __('User Notifications', 'folio'),
            array('description' => __('Display latest notifications for current user', 'folio'))
        );
    }
    ...
    public function widget($args, $instance) {
        ...
        $title = !empty($instance['title']) ? $instance['title'] : __('My Notifications', 'folio');
        ...
        if (empty($notifications)) {
            echo '<p class="no-notifications">' . esc_html__('No notifications', 'folio') . '</p>';
        } else {
            echo '<div class="folio-widget-notifications">';
            
            if ($unread_count > 0) {
                echo '<div class="unread-summary">' . sprintf(esc_html__('You have %d unread notifications', 'folio'), $unread_count) . '</div>';
            }
            ...
            echo '<a href="' . esc_url(home_url('/user-center/?tab=notifications')) . '" class="view-all-notifications">' . esc_html__('View all notifications', 'folio') . '</a>';
            echo '</div>';
    ...
    public function form($instance) {
        ...
        <p>
            <label for="<?php echo esc_attr($this->get_field_id('limit')); ?>"><?php esc_html_e('Number of items:', 'folio'); ?></label>
