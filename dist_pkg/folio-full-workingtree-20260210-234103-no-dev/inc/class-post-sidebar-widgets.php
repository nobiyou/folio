...
class folio_Post_Tags_Widget extends WP_Widget {
    ...
    public function __construct() {
        parent::__construct(
            ...
            __('Post Tags', 'folio'),
            array('description' => __('Display tags for the current post', 'folio'))
        );
    }
    ...
    public function widget($args, $instance) {
        ...
        $title_raw = !empty($instance['title']) ? $instance['title'] : '';
        $title = $title_raw !== '' ? __($title_raw, 'folio') : __('Tags', 'folio');
        $post_id = get_the_ID();
    ...
    public function form($instance) {
        ...
        ?>
        <p>
            <label for="<?php echo esc_attr($this->get_field_id('title')); ?>"><?php esc_html_e('Title:', 'folio'); ?></label>
...
class folio_Popular_Posts_Widget extends WP_Widget {
    ...
    public function __construct() {
        parent::__construct(
            ...
            __('Recent Popular Posts', 'folio'),
            array('description' => __('Display recent popular posts (sorted by views)', 'folio'))
        );
    }
    ...
    public function widget($args, $instance) {
        $title_raw = !empty($instance['title']) ? $instance['title'] : '';
        $title = $title_raw !== '' ? __($title_raw, 'folio') : __('Popular Posts', 'folio');
        ...
        while ($popular_posts->have_posts()) {
            ...
            <li class="popular-post-item">
                <a href="<?php echo esc_url(get_permalink()); ?>" class="flex gap-3 group">
                    ...
                    echo folio_render_thumbnail_block(get_the_ID(), 'thumbnail', array(
                        'wrapper_class' => 'post-thumbnail flex-shrink-0 w-16 h-16 rounded overflow-hidden',
                    ...
                    <div class="post-info flex-1 min-w-0">
                        ...
                        <div class="post-meta flex items-center gap-3 text-xs text-gray-500">
                            ...
                            <span class="text-gray-400"><?php echo esc_html(folio_format_post_date(get_the_ID())); ?></span>
                        </div>
                    </div>
                </a>
            </li>
    ...
    public function form($instance) {
        ...
        <p>
            <label for="<?php echo esc_attr($this->get_field_id('limit')); ?>"><?php esc_html_e('Number of posts:', 'folio'); ?></label>
        ...
        <p>
            <label for="<?php echo esc_attr($this->get_field_id('days')); ?>"><?php esc_html_e('Recent days:', 'folio'); ?></label>
