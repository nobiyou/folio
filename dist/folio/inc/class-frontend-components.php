...
class folio_Frontend_Components {
    ...
    public static function render_membership_badge($post_id, $context = 'list', $options = array()) {
        ...
        if ($can_access) {
            ...
            $status_text = __('Unlocked', 'folio');
            $tooltip_text = sprintf(__('You can view this %s exclusive content', 'folio'), $level_name);
            ...
            if ($user_id && $user_membership) {
                ...
                $status_text = __('Upgrade required', 'folio');
                $tooltip_text = sprintf(
                    /* translators: 1: current level, 2: required level */
                    __('Current level: %1$s. Upgrade to %2$s to view.', 'folio'),
                    $current_level_name,
                    $level_name
                );
            } else {
                $badge_text = sprintf(__('Requires %s', 'folio'), $level_name);
                $status_text = __('Login required', 'folio');
                $tooltip_text = sprintf(__('Please log in and upgrade to %s to view this exclusive content', 'folio'), $level_name);
            }
        ...
        if ($options['show_tooltip']) {
            if (!$can_access) {
                ...
                $enhanced_tooltip .= "\n\n" . sprintf(__('%s member benefits:', 'folio'), $level_name) . ' ' . $benefits;
                
                // 添加升级提示
                if ($user_id) {
                    $enhanced_tooltip .= "\n\n" . sprintf(__('Click to upgrade to %s membership', 'folio'), $level_name);
                } else {
                    $enhanced_tooltip .= "\n\n" . __('Click to log in and upgrade membership', 'folio');
                }
            } else {
                $enhanced_tooltip .= "\n\n" . __('Thank you for your support!', 'folio');
            }
        }
        ...
        <div class="<?php echo esc_attr(implode(' ', $prompt_classes)); ?>" 
            ...
            <div class="folio-prompt-modern-header">
                <h3 class="folio-prompt-modern-title">
                    <?php
                    echo $options['custom_title']
                        ? esc_html($options['custom_title'])
                        : esc_html(sprintf(__('Upgrade to %s to unlock exclusive content', 'folio'), $level_name));
                    ?>
                </h3>
            </div>
                ...
                <div class="folio-vip-card-content-wrapper">
                    <div class="folio-vip-card-left">
                        <div class="folio-vip-card-header">
                            <h4 class="folio-vip-card-title"><?php echo esc_html(sprintf(__('%s Member', 'folio'), $level_name)); ?></h4>
                        </div>
                        <div class="folio-vip-card-content">
                            <p class="folio-vip-card-description">
                                ...
                                echo esc_html(
                                    sprintf(
                                        /* translators: %d: number of benefits */
                                        __('Enjoy %d premium benefits and unlock more featured albums', 'folio'),
                                        count($benefits_comparison)
                                    )
                                );
                                ?>
                            </p>
                    ...
                    <div class="folio-vip-card-right">
                        <div class="folio-vip-card-pricing">
                            <div class="folio-vip-price-label"><?php echo esc_html(sprintf(__('%s Member', 'folio'), $level_name)); ?></div>
            ...
            <div class="folio-prompt-comparison-section">
                <h5 class="folio-comparison-title"><?php echo esc_html(sprintf(__('%s Exclusive Benefits', 'folio'), $level_name)); ?></h5>
                <div class="folio-comparison-table-wrapper">
                    <table class="folio-comparison-table">
                        <thead>
                            <tr>
                                <th class="folio-col-feature"><?php esc_html_e('Exclusive Benefit', 'folio'); ?></th>
                                <th class="folio-col-normal"><?php esc_html_e('Regular User', 'folio'); ?></th>
                                <?php if ($required_level === 'svip') : ?>
                                <th class="folio-col-vip"><?php esc_html_e('VIP User', 'folio'); ?></th>
                                <th class="folio-col-svip"><?php esc_html_e('SVIP User', 'folio'); ?></th>
                                <?php else : ?>
                                <th class="folio-col-vip"><?php esc_html_e('VIP User', 'folio'); ?></th>
                                <?php endif; ?>
                            </tr>
            ...
            <div class="folio-prompt-modern-footer">
                <div class="folio-current-status-modern">
                    <span class="folio-status-text"><?php echo esc_html(sprintf(__('Current Level: %s', 'folio'), $user_membership['name'])); ?></span>
    ...
    private static function get_default_membership_comparison($level) {
        $comparisons = array(
            'vip' => array(
                ...
                array('name' => __('Ad-free browsing', 'folio'), 'normal' => __('With ads', 'folio'), 'vip' => '✓'),
                ...
                array('name' => __('HD image download', 'folio'), 'normal' => __('Limited download', 'folio'), 'vip' => '✓'),
                array('name' => __('Article bookmarking', 'folio'), 'normal' => '×', 'vip' => '✓'),
                array('name' => __('Priority comment display', 'folio'), 'normal' => __('Standard sorting', 'folio'), 'vip' => '✓'),
                array('name' => __('Exclusive content push', 'folio'), 'normal' => '×', 'vip' => '✓'),
                array('name' => __('Members-only events', 'folio'), 'normal' => '×', 'vip' => '✓')
            ),
            'svip' => array(
                array('name' => __('View all exclusive content', 'folio'), 'normal' => '×', 'vip' => __('Partial content', 'folio'), 'svip' => '✓'),
                array('name' => __('Ad-free browsing', 'folio'), 'normal' => __('With ads', 'folio'), 'vip' => '✓', 'svip' => '✓'),
                array('name' => __('24-hour dedicated support', 'folio'), 'normal' => __('Business hours', 'folio'), 'vip' => __('Priority support', 'folio'), 'svip' => '✓'),
                array('name' => __('Exclusive SVIP badge', 'folio'), 'normal' => '×', 'vip' => __('VIP badge', 'folio'), 'svip' => '✓'),
                array('name' => __('Exclusive HD resources', 'folio'), 'normal' => '×', 'vip' => __('Standard resources', 'folio'), 'svip' => '✓'),
                array('name' => __('Early access to new features', 'folio'), 'normal' => '×', 'vip' => '×', 'svip' => '✓'),
                array('name' => __('Unlimited downloads', 'folio'), 'normal' => __('Limited download', 'folio'), 'vip' => __('Limited quota', 'folio'), 'svip' => '✓'),
                array('name' => __('Exclusive content customization', 'folio'), 'normal' => '×', 'vip' => '×', 'svip' => '✓'),
                array('name' => __('SVIP exclusive events', 'folio'), 'normal' => '×', 'vip' => __('Partial events', 'folio'), 'svip' => '✓')
            )
        );
    ...
    public static function get_membership_price($level) {
        ...
        $default_prices = array(
            'vip' => __('¥68/month', 'folio'),
            'svip' => __('¥128/month', 'folio')
        );
    ...
    public static function render_upgrade_button($required_level, $options = array()) {
        ...
        // 默认选项
        $defaults = array(
            'text' => sprintf(__('Upgrade to %s', 'folio'), $level_name),
            'size' => 'normal', // small, normal, large
    ...
    private static function get_membership_benefits($level) {
        $benefits = array(
            ...
            'svip' => array(
                ...
                __('Ad-free browsing', 'folio'),
                __('24-hour dedicated support', 'folio'),
                __('Exclusive SVIP badge', 'folio'),
                __('Exclusive HD resources', 'folio'),
    ...
    private static function get_level_benefits_summary($level) {
        $summaries = array(
            'vip' => __('Exclusive content, ad-free, priority support', 'folio'),
            'svip' => __('All content, 24h support, exclusive resources', 'folio')
        );
    ...
    public function upgrade_button_shortcode($atts) {
        ...
        $options = array(
            'text' => $atts['text'] ?: sprintf(__('Upgrade to %s', 'folio'), ($atts['level'] === 'svip' ? 'SVIP' : 'VIP')),
            'size' => $atts['size'],
            'style' => $atts['style'],
            'icon' => $atts['icon'] === 'true',
    ...
    public function ajax_get_permission_status() {
        ...
        if ($nonce === '' || !wp_verify_nonce($nonce, 'folio_frontend_components')) {
        ...
        if (is_singular('post')) {
            ...
            wp_localize_script('folio-frontend-components', 'folioComponents', array(
                ...
                'strings' => array(
                    ...
                    'login_required' => __('Please login first', 'folio'),
                    'upgrade_required' => __('Membership upgrade required', 'folio'),
