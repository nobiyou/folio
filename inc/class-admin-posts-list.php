...
function folio_add_featured_image_column($columns) {
    ...
    foreach ($columns as $key => $value) {
        ...
        if ('title' === $key) { // 在标题列之后插入
            $new_columns['featured_image'] = __('头图', 'folio'); // 列标题
        }
    }
    return $new_columns;
}
...
function folio_add_vip_bulk_actions($bulk_actions) {
    ...
    // 在现有批量操作中追加
    $bulk_actions['folio_set_vip']  = __('设置为 VIP 专享', 'folio');
    $bulk_actions['folio_set_svip'] = __('设置为 SVIP 专享', 'folio');
    $bulk_actions['folio_clear_vip'] = __('取消会员专享', 'folio');
...
function folio_vip_bulk_admin_notice() {
    ...
    if (!empty($_REQUEST['folio_vip_set'])) {
        ...
        printf(
            ...
            esc_html(sprintf(__('已将 %d 篇文章设置为 VIP 专享。', 'folio'), $count))
        );
    }
    ...
    if (!empty($_REQUEST['folio_svip_set'])) {
        ...
        printf(
            '<div class="notice notice-success is-dismissible"><p>%s</p></div>',
            esc_html(sprintf(__('已将 %d 篇文章设置为 SVIP 专享。', 'folio'), $count))
        );
    }
    ...
    if (!empty($_REQUEST['folio_vip_cleared'])) {
        ...
        printf(
            '<div class="notice notice-success is-dismissible"><p>%s</p></div>',
            esc_html(sprintf(__('已取消 %d 篇文章的 VIP 专享状态。', 'folio'), $count))
        );
    }
}
            ...
            if ($row.length > 0) {
                ...
                if ($dateCell.length > 0) {
                    ...
                    if ($dateCell.find('.folio-edited-badge').length === 0) {
                        var badge = '<br><span class="folio-edited-badge" title="此文章已被编辑">' +
                            '<span class="folio-edited-icon">✏️</span> ' +
                            '<span class="folio-edited-text">已编辑</span>' +
                            '</span>';
 ...
 * @return array 修改后的列
 */
function folio_add_views_column($columns) {
    $new_columns = array();
    foreach ($columns as $key => $value) {
        $new_columns[$key] = $value;
        if ('date' === $key) { // 在日期列之前插入
            $new_columns['views'] = __('浏览量', 'folio');
        }
    }
    return $new_columns;
}
add_filter('manage_posts_columns', 'folio_add_views_column');
 ...
 * @param int    $post_id 当前文章的 ID
 */
function folio_display_views_column($column, $post_id) {
    if ('views' === $column) {
        // 使用 folio_Post_Stats 类的方法获取浏览量
        if (class_exists('folio_Post_Stats')) {
            $views = folio_Post_Stats::get_views($post_id);
            // 获取原始数值用于排序
            $raw_views = (int) get_post_meta($post_id, 'views', true);
            echo '<span class="folio-views-count" data-views="' . esc_attr($raw_views) . '">' . esc_html($views) . '</span>';
        } else {
            // 如果类不存在，直接获取 meta 值
            $views = (int) get_post_meta($post_id, 'views', true);
            echo '<span class="folio-views-count">' . esc_html($views ?: 0) . '</span>';
        }
    }
}
add_action('manage_posts_custom_column', 'folio_display_views_column', 10, 2);
 ...
 * @return array
 */
function folio_make_views_column_sortable($columns) {
    $columns['views'] = 'views';
    return $columns;
}
add_filter('manage_edit-post_sortable_columns', 'folio_make_views_column_sortable');
 ...
 */
function folio_sort_posts_by_views($query) {
...
add_action('pre_get_posts', 'folio_sort_posts_by_views');

/**
 * 添加浏览量列样式
 */
function folio_admin_views_column_styles() {
    $screen = get_current_screen();
    if (!$screen || $screen->post_type !== 'post' || $screen->base !== 'edit') {
        return;
    }
...
add_action('admin_head', 'folio_admin_views_column_styles');