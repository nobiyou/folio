...
/**
 * Get ribbon type for a portfolio item
 *
...
/**
 * Get portfolio category for display
 *
 * @param int $post_id Post ID (optional)
 * @return string Category name or empty string
 */
function folio_get_portfolio_category($post_id = null) {
    if (!$post_id) {
        $post_id = get_the_ID();
    }
 ...
 */
function folio_render_post_tags_widget($post_id = null) {
    if (!$post_id) {
        $post_id = get_the_ID();
    }
    ...
    ?>
    <div class="post-tags-widget mb-8">
        <h3 class="widget-title text-xs uppercase font-bold tracking-widest mb-4 text-gray-700">
            <?php esc_html_e('Tags', 'folio'); ?>
        </h3>
        <div class="tags-list flex flex-wrap gap-2">
            <?php foreach ($tags as $tag) : ?>
                <a href="<?php echo esc_url(get_tag_link($tag->term_id)); ?>" 
                   class="tag-item inline-block px-3 py-1.5 text-xs font-medium bg-gray-100 hover:bg-gray-200 text-gray-700 rounded transition-colors">
 ...
 * 渲染最近热门文章小工具
 * 
 * @param int $limit 显示文章数量，默认5篇
 * @param int $days 最近天数，默认30天
 */
function folio_render_popular_posts_widget($limit = 5, $days = 30) {
    $args = array(
        'post_type' => 'post',
    ...
    <div class="popular-posts-widget">
        <h3 class="widget-title text-xs uppercase font-bold tracking-widest mb-4 text-gray-700">
            <?php esc_html_e('Popular Posts', 'folio'); ?>
        </h3>
                    ...
                    <a href="<?php echo esc_url(get_permalink()); ?>" class="flex gap-3 group">
                        ...
                        <div class="post-info flex-1 min-w-0">
                            ...
                            <div class="post-meta flex items-center gap-3 text-xs text-gray-500">
                                ...
                                <span class="text-gray-400"><?php echo get_the_date('M j'); ?></span>
                            </div>
                        </div>
                    </a>
                </li>
 ...
 * @return int 图片数量
 */
function folio_get_post_image_count($post_id = null) {
    if (!$post_id) {
        $post_id = get_the_ID();
    }
