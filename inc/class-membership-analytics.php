...
class folio_Membership_Analytics {
    ...
    public function render_analytics_page() {
        ...
        ?>
        <div class="wrap">
            <h1><?php esc_html_e('会员内容统计分析', 'folio'); ?></h1>
            
            <!-- 统计卡片 -->
            <div class="folio-analytics-cards">
                <div class="analytics-card">
                    <h3><?php esc_html_e('内容统计', 'folio'); ?></h3>
                    <div class="card-content">
                        <div class="stat-item">
                            <span class="stat-number"><?php echo esc_html($content_stats['total_protected_articles']); ?></span>
                            <span class="stat-label"><?php esc_html_e('会员专属文章', 'folio'); ?></span>
                        </div>
                        <div class="stat-item">
                            <span class="stat-number"><?php echo esc_html($content_stats['vip_articles']); ?></span>
                            <span class="stat-label"><?php esc_html_e('VIP文章', 'folio'); ?></span>
                        </div>
                        <div class="stat-item">
                            <span class="stat-number"><?php echo esc_html($content_stats['svip_articles']); ?></span>
                            <span class="stat-label"><?php esc_html_e('SVIP文章', 'folio'); ?></span>
                        </div>
                    </div>
                </div>
                
                <div class="analytics-card">
                    <h3><?php esc_html_e('访问统计 (30天)', 'folio'); ?></h3>
                    <div class="card-content">
                        <div class="stat-item">
                            <span class="stat-number"><?php echo esc_html($access_stats['total_access']); ?></span>
                            <span class="stat-label"><?php esc_html_e('总访问次数', 'folio'); ?></span>
                        </div>
                        <div class="stat-item">
                            <span class="stat-number"><?php echo esc_html($access_stats['member_access']); ?></span>
                            <span class="stat-label"><?php esc_html_e('会员访问', 'folio'); ?></span>
                        </div>
                        <div class="stat-item">
                            <span class="stat-number"><?php echo esc_html($access_stats['denied_access']); ?></span>
                            <span class="stat-label"><?php esc_html_e('被拒绝访问', 'folio'); ?></span>
                        </div>
                    </div>
                </div>
                
                <div class="analytics-card">
                    <h3><?php esc_html_e('转化分析 (30天)', 'folio'); ?></h3>
                    <div class="card-content">
                        <div class="stat-item">
                            <span class="stat-number"><?php echo esc_html($conversion_stats['conversion_rate']); ?>%</span>
                            <span class="stat-label"><?php esc_html_e('转化率', 'folio'); ?></span>
                        </div>
                        <div class="stat-item">
                            <span class="stat-number"><?php echo esc_html($conversion_stats['conversions']); ?></span>
                            <span class="stat-label"><?php esc_html_e('转化次数', 'folio'); ?></span>
                        </div>
                        <div class="stat-item">
                            <span class="stat-number"><?php echo esc_html($conversion_stats['bounce_rate']); ?>%</span>
                            <span class="stat-label"><?php esc_html_e('跳出率', 'folio'); ?></span>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <!-- 图表区域 -->
            <div class="folio-analytics-charts">
                <div class="chart-container">
                    <h3><?php esc_html_e('访问趋势', 'folio'); ?></h3>
                    <canvas id="accessTrendChart" width="400" height="200"></canvas>
                </div>
                
                <div class="chart-container">
                    <h3><?php esc_html_e('转化趋势', 'folio'); ?></h3>
                    <canvas id="conversionTrendChart" width="400" height="200"></canvas>
                </div>
            </div>
            
            <!-- 热门文章表格 -->
            <div class="folio-analytics-table">
                <h3><?php esc_html_e('热门受保护文章 (30天)', 'folio'); ?></h3>
                <table class="wp-list-table widefat fixed striped">
                    <thead>
                        <tr>
                            <th><?php esc_html_e('文章标题', 'folio'); ?></th>
                            <th><?php esc_html_e('总访问', 'folio'); ?></th>
                            <th><?php esc_html_e('允许访问', 'folio'); ?></th>
                            <th><?php esc_html_e('拒绝访问', 'folio'); ?></th>
                            <th><?php esc_html_e('转化次数', 'folio'); ?></th>
                            <th><?php esc_html_e('转化率', 'folio'); ?></th>
                        </tr>
                    </thead>
                    <tbody>
            ...
            <!-- 导出按钮 -->
            <div class="folio-analytics-actions">
                <button type="button" class="button button-primary" id="exportAnalytics">
                    <?php esc_html_e('导出报告', 'folio'); ?>
                </button>
            </div>
        </div>
        
        <style>
        .folio-analytics-cards {
            display: grid;
        ...
        .folio-analytics-table {
            margin: 20px 0;
        }
        
        .folio-analytics-actions {
            margin: 20px 0;
        }
        ...
        jQuery(document).ready(function($) {
            ...
            $('#exportAnalytics').on('click', function() {
                window.location.href = ajaxurl + '?action=folio_export_analytics&_wpnonce=' + 
                    '<?php echo wp_create_nonce('folio_export_analytics'); ?>';
            });
    ...
    public function ajax_export_analytics() {
        if (!wp_verify_nonce($_GET['_wpnonce'] ?? '', 'folio_export_analytics')) {
            wp_die(__('安全验证失败', 'folio'));
        }
        
        if (!current_user_can('manage_options')) {
            wp_die(__('权限不足', 'folio'));
        }
    ...
    private function export_analytics_report() {
        ...
        // 内容统计
        fputcsv($output, array('会员内容统计'));
        fputcsv($output, array('指标', '数值'));
        fputcsv($output, array('会员专属文章总数', $content_stats['total_protected_articles']));
        fputcsv($output, array('VIP文章数量', $content_stats['vip_articles']));
        fputcsv($output, array('SVIP文章数量', $content_stats['svip_articles']));
        fputcsv($output, array('免费文章数量', $content_stats['free_articles']));
        fputcsv($output, array(''));
        
        // 访问统计
        fputcsv($output, array('访问统计 (30天)'));
        fputcsv($output, array('指标', '数值'));
        fputcsv($output, array('总访问次数', $access_stats['total_access']));
        fputcsv($output, array('会员访问次数', $access_stats['member_access']));
        fputcsv($output, array('非会员访问次数', $access_stats['non_member_access']));
        fputcsv($output, array('被拒绝访问次数', $access_stats['denied_access']));
        fputcsv($output, array('独立访客数', $access_stats['unique_visitors']));
        fputcsv($output, array(''));
        
        // 转化统计
        fputcsv($output, array('转化分析 (30天)'));
        fputcsv($output, array('指标', '数值'));
        fputcsv($output, array('转化次数', $conversion_stats['conversions']));
        fputcsv($output, array('跳出会话数', $conversion_stats['bounce_sessions']));
        fputcsv($output, array('转化率 (%)', $conversion_stats['conversion_rate']));
        fputcsv($output, array('跳出率 (%)', $conversion_stats['bounce_rate']));
        fputcsv($output, array(''));
        
        // 热门文章
        fputcsv($output, array('热门受保护文章 (30天)'));
        fputcsv($output, array('文章标题', '总访问', '允许访问', '拒绝访问', '转化次数', '转化率 (%)'));
